...
config = {'figures': [{'charts':
          ...
          'states': ['running'],  # locust states for which we'll plot the graphs
          'categories':['CHECK','RETRIEVE','SCORE','SUBMIT'],
...
for state in config['states']:
    ...
    for category in config['categories']:
        data_sources[state][category] = {}
        for figure_data in config['figures']:
            # initialization of figure
            new_figure = figure(title=figure_data['title'].format(category),plot_height=400,plot_width=820)
            ...
            for chart in figure_data['charts']:
                ...
                scatter = marker(x=[0], y=[0], color=chart['color'], size=8, legend=chart['legend'])
                line = new_figure.line(x=[0], y=[0], color=chart['color'], line_width=1, legend=chart['legend'])
                # adding data source for markers and line
                data_sources[state][category][chart['key']] = scatter.data_source = line.data_source
...
#         return
#     resp_data = resp.json()
#     datas = resp_data['stats'][:-1]  # Abandon the record for total
...
#             #print data
#             #print "----------------------------------------------------------"
#             for key,data_source in six.iteritems(data_sources[resp_data['state']][data['name']]):
...
def update():
    try:
        with open('result.csv','r') as f:
            ...
            for row in reader:
                if row['name'] in ('LOGIN','Total'):
                    continue
                # print row
                # print "----------------------------------------------------------"
                for key,data_source in six.iteritems(data_sources['running'][row['name']]):
                    data_source.data['x'].append(row[requests_key])
                    data_source.data['y'].append(row[key])
                    data_source.trigger('data', data_source.data, data_source.data)
                # print data_sources
                # print "----------------------------------------------------------"
    except IOError:
        print "Cannot read the csv file."
        return
    finally:
        f.close()
